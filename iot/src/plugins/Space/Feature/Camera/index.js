import { SDK_EVENT_NAME_ENUM, TAG_VISIBLE_COMPUTE_RULE_ENUM } from '@/constants'
...
export class Camera {
  ...
  init() {
    ...
    this.qspace.panoramaCamera.addEventListener(
      SDK_EVENT_NAME_ENUM.PANORAMA_CAMERA_ROTATION,
      () => {
        this.space.feature.tag.computeTagVisible()
      }
    )
  }
  ...
  updateZoom() {
    ...
    if (
      this.space.modelType === 'purepano' ||
      this.space.viewMode === 'panorama'
    ) {
      dollhouseCamera.disableDistanceZoomControl()
      dollhouseCamera.enableZoomControl()
    } else {
      // 实拍、虚拟、倾斜摄影，相机模式为 3d 或平面时，使用“距离缩放”
  ...
  lookAtQuaternion(options) {
    const { panoramaCamera } = this.qspace

    panoramaCamera.disableControl()
    panoramaCamera.lookAtQuaternion({
      ...
      complete: () => {
        ...
        panoramaCamera.enableControl()
        this.space.emit('space.camera.look.at.quaternion.complete', options)
      },
    })
  }
